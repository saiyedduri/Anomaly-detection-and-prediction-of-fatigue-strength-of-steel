...
class Data_preprocessing:
    ...
    def split_dataset(self,X,y):
        ...
        ========================================================================
        '''
        X_train, X_test = np.split(X.sample(frac = 1,random_state = 0).values,[int(0.8*len(X))])
        y_train, y_test = np.split(y.sample(frac = 1,random_state = 0).values,[int(0.8*len(y))])
...
class MeanSquaredError_loss(LossCalculation):
    ...
    def back_prop(self, derivatives, y):
        ...
        outputsPerSample = len(derivatives[0]) 
