'''
========================================================================
Test_file: test_CompleteANN_Adam_optimizer.py
Test_type: Gradient checking
Aim: To validate one complete training step using adam optimizer
========================================================================   
'''
...
class Adam_Optimizer:
   ...
   def parameters_update(self,layer):
      ...
      ------------------------------------------------------------------------
      Output:
      weight_momentum_M: The update to the weight momentum @ mean is done by retaining a fraction of the previous momentum and adding it with the fraction of derivatives
      weight_momentum_V: The update to the weight momentum @ varience is done by retaining a fraction of the previous momentum and adding it with the fraction of squared derivatives
      C_weight_momentum_M & V: This is a bias correction mechanism that is applied in both mean and variance to compensate the zerod values during the initial steps. This is 
      ...
      ========================================================================
      '''
      if not hasattr(layer, 'weight_momentum_M'):
         layer.weight_momentum_M = np.zeros_like(layer.weights)  # Assigning momentum arrays by checking If there exist any momentum attribute in the dense layers
         layer.weight_momentum_V = np.zeros_like(layer.weights)  # Initial value will be assigned as zeros for both weights and biases @ mean and varience
         layer.bias_momentum_M = np.zeros_like(layer.biases)
         layer.bias_momentum_V = np.zeros_like(layer.biases)
      
      layer.weight_momentum_M = self.beta1 * layer.weight_momentum_M + (1 - self.beta1) * layer.weights_derv
      ...
      layer.weight_momentum_V = self.beta2 * layer.weight_momentum_V + (1 - self.beta2) * layer.weights_derv**2
...
def back_prop_lossfunc(loss_function, activation_function, y , epsilon):
   ...
   ========================================================================
   '''
   loss_back_prop = loss_function.back_prop(activation_function.output, y) # Analytical solution
...
def back_prop_activationfunc(activation_function, derivative, Dense_layer, epsilon):
   ...
   ========================================================================
   '''
   actfun_back_prop = activation_function.back_prop(derivative.inputs_derv) # Analytical solution
...
def optimizer_learningRate(optimizer,ExpAdam_Current_LR):
   '''
   ========================================================================
   Description:
   Validating the expected current learning rate with obtained one during single training step
   ------------------------------------------------------------------------
   Parameters:
   optimizer: Optimizer used -> Adam
   ExpAdam_Current_LR: Expected learning rate; dtype -> float
   ------------------------------------------------------------------------
   Return:
   Assertion results for current learning rate
   ========================================================================
   '''
   Adam_Current_LR = optimizer.learning_R_update() # Analytical solution

   # Assertion
   #------------------------------------------------------------------------
   assert abs(Adam_Current_LR - ExpAdam_Current_LR) < 0.0001, 'test failed' 

def optimizer_Adam(optimizer,Dense_layer,ExpAdam_Current_LR,GradCheck_weightsBP,GradCheck_biasesBP,epsilon_op,beta1,beta2,itr):
   '''
   ========================================================================
   Description:
   ...
   Adam_DL_Weights,Adam_DL_biases = Dense_layer.weights, Dense_layer.biases # Analytical solution

   # Optimization computed using numerical gradient results
   #------------------------------------------------------------------------
   Adam_DL_weight_momentum_M = (1 - beta1) * GradCheck_weightsBP
   Adam_DL_bias_momentum_M = (1 - beta1) * GradCheck_biasesBP
      
   Adam_DL_C_weight_momentum_M = Adam_DL_weight_momentum_M / (1 - beta1 **(itr + 1))
   Adam_DL_C_bias_momentum_M = Adam_DL_bias_momentum_M / (1 - beta1 **(itr + 1))    

   Adam_DL_weight_momentum_V = (1 - beta2) * GradCheck_weightsBP**2
   Adam_DL_bias_momentum_V = (1 - beta2) * GradCheck_biasesBP**2
      
   Adam_DL_C_weight_momentum_V = Adam_DL_weight_momentum_V / (1 - beta2 **(itr + 1)) 
   Adam_DL_C_bias_momentum_V = Adam_DL_bias_momentum_V / (1 - beta2 **(itr + 1)) 

   ExpAdam_DL_Weights = np.ones((Dense_layer.weights.shape)) -ExpAdam_Current_LR * Adam_DL_C_weight_momentum_M/ (np.sqrt(Adam_DL_C_weight_momentum_V) + epsilon_op) 
   ExpAdam_DL_biases = 1 -ExpAdam_Current_LR * Adam_DL_C_bias_momentum_M / (np.sqrt(Adam_DL_C_bias_momentum_V) + epsilon_op)

   # Assertion
   #------------------------------------------------------------------------
   np.testing.assert_array_almost_equal(Adam_DL_Weights ,ExpAdam_DL_Weights), 'test failed'
   np.testing.assert_array_almost_equal(Adam_DL_biases ,ExpAdam_DL_biases), 'test failed'
...
def read_parameters():
   ...
   if cwd == 'test_CompleteANN_Adam_optimizer':                   # If tested from the current directory
      filename = 'parameters.txt'
   elif cwd == 'test_ANN':                                        # If tested from test_ANN
      open_path = os.path.abspath('test_CompleteANN_Adam_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
   elif cwd == 'test_ML':                                         # If tested from test_ML
      open_path = os.path.abspath('test_ANN/test_CompleteANN_Adam_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
   ...
   #------------------------------------------------------------------------
   param = [(np.array([input[0]]), depFeat[0],0.001,0.000999990000,1e-7,0.9,0.999,1),(np.array([input[1]]),depFeat[1], 0.001,0.000999990000,1e-7,0.9,0.999,1),\
   (np.array([input[2]]), depFeat[2],0.001,0.000999990000,1e-7,0.9,0.999,1),(np.array([input[3]]),depFeat[3],0.001,0.000999990000,1e-7,0.9,0.999,1),\
   (np.array([input[4]]), depFeat[4],0.001,0.000999990000,1e-7,0.9,0.999,1)]
   return param

# Defining multiple test cases for testing
#------------------------------------------------------------------------
@pytest.mark.parametrize('input,y,epsilon,Exp_LR,epsilon_op,beta1,beta2,itr', read_parameters())

def test_CompleteANN_Adam_Optimizer(input,y,epsilon,Exp_LR,epsilon_op,beta1,beta2,itr):
   '''
   ========================================================================
   Description:
   Test one complete operation of the neural network with adam optimizer by using numerical gradient
   ------------------------------------------------------------------------
   Parameters:
   ...
   optimizer = Adam_Optimizer(learning_R = 1e-3, learning_R_decay = 1e-5, epsilon = 1e-7, beta1 = 0.9, beta2 = 0.999)
   ...
   # Checking Adam Optimizer for parameters updates in Dense layer 1
   optimizer_Adam(optimizer,dense_layer_1,Exp_LR,GC_weightsDL1,GC_biasesDL1,epsilon_op,beta1,beta2,itr)
   
   # Checking Adam Optimizer for parameters updates in Dense layer 2
   optimizer_Adam(optimizer,dense_layer_2,Exp_LR,GC_weightsDL2,GC_biasesDL2,epsilon_op,beta1,beta2,itr)

   # Checking Adam Optimizer for parameters updates in Dense layer 3
   optimizer_Adam(optimizer,dense_layer_3,Exp_LR,GC_weightsDL3,GC_biasesDL3,epsilon_op,beta1,beta2,itr) 