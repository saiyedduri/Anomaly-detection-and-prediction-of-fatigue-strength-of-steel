...
class Data_preprocessing:
   ...
   def import_dataset(self,data):
      ...
      The dependent feature must be the last column of the dataset
      ------------------------------------------------------------------------
      Return:
   ...
   def split_dataset(self,scaled_X,scaled_y):
      ...
      ========================================================================
      '''
      X_train, X_test = np.split(scaled_X.sample(frac = 1,random_state = 0).values,[int(0.8*len(scaled_X))])
      y_train, y_test = np.split(scaled_y.sample(frac = 1,random_state = 0).values,[int(0.8*len(scaled_y))])
...
class LossCalculation:
   ...
   def calculate_loss(self,output, y):
      ...
      ========================================================================
      '''
      lossPer_sample = self.forward_prop(output, y)
...
class RootMeanSquaredError_loss(LossCalculation):
   ...
   def back_prop(self, derivatives, y):
      ...
      self.inputs_derv = (-1 * (y - derivatives) / outputsPerSample) / np.sqrt((y - derivatives)**2 / outputsPerSample) 
