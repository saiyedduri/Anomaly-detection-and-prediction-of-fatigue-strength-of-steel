...
class RMSProp_Optimizer:
   ...
   def parameters_update(self,layer):
      ...
      layer.weight_history = self.rho * layer.weight_history + (1 - self.rho) * layer.weights_derv**2
      layer.bias_history = self.rho * layer.bias_history + (1 - self.rho) * layer.biases_derv**2

      layer.weights += -self.C_learning_R * layer.weights_derv / (np.sqrt(layer.weight_history) + self.epsilon) # Epsilon - to avoid division by 0
      layer.biases += -self.C_learning_R * layer.biases_derv / (np.sqrt(layer.bias_history) + self.epsilon)
...
def back_prop_activationfunc(activation_function, derivative, Dense_layer, epsilon):
   ...
   ========================================================================
   '''
   actfun3_back_prop = activation_function.back_prop(derivative.inputs_derv)
...
def optimizer_learningRate(optimizer,ExpRMSP_Current_LR):
   '''
   ========================================================================
   Description:
   ...
   ========================================================================
   '''
   RMSP_Current_LR = optimizer.learning_R_update()

   # Assertion
   #------------------------------------------------------------------------
   assert abs(RMSP_Current_LR - ExpRMSP_Current_LR) < 0.0001, 'test failed' 


def optimizer_RMSP(optimizer,Dense_layer,ExpRMSP_Current_LR,GradCheck_weightsBP,GradCheck_biasesBP,epsilon_op,rho):
   '''
   ========================================================================
   Description:
   ...
   RMS_DL_Weights,RMS_DL1_biases = Dense_layer.weights, Dense_layer.biases

   # Optimization computed using numerical gradient results
   #------------------------------------------------------------------------
   RMS_DL_weight_history =  (1 - rho) * GradCheck_weightsBP**2
   RMS_DL_bias_history =  (1 - rho) * GradCheck_biasesBP**2
   ExpRMS_DL_Weights = np.ones((Dense_layer.weights.shape)) -ExpRMSP_Current_LR * GradCheck_weightsBP / (np.sqrt(RMS_DL_weight_history)+epsilon_op)
   ExpRMS_DL_biases = 1 -ExpRMSP_Current_LR * GradCheck_biasesBP / (np.sqrt(RMS_DL_bias_history)+epsilon_op)
   
   # Assertion
   #------------------------------------------------------------------------
   np.testing.assert_array_almost_equal(RMS_DL_Weights ,ExpRMS_DL_Weights), 'test failed'
   np.testing.assert_array_almost_equal(RMS_DL1_biases ,ExpRMS_DL_biases), 'test failed'
...
def read_parameters():
   ...
   if cwd == 'test_CompleteANN_RMSProp_optimizer':                # If tested from the current directory
      filename = 'parameters.txt'
   elif cwd == 'test_ANN':                                        # If tested from test_ANN
      open_path = os.path.abspath('test_CompleteANN_RMSProp_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
   elif cwd == 'test_ML':                                         # If tested from test_ML
      open_path = os.path.abspath('test_ANN/test_CompleteANN_RMSProp_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
   ...
   #------------------------------------------------------------------------
   param = [(np.array([input[0]]), depFeat[0],0.001,0.00099990001,1e-7,0.9),(np.array([input[1]]),depFeat[1], 0.001,0.00099990001,1e-7,0.9),\
   (np.array([input[2]]), depFeat[2],0.001,0.00099990001,1e-7,0.9),(np.array([input[3]]),depFeat[3],0.001,0.00099990001,1e-7,0.9),\
   (np.array([input[4]]), depFeat[4],0.001,0.00099990001,1e-7,0.9)]
   return param

# Defining multiple test cases for testing
#------------------------------------------------------------------------
@pytest.mark.parametrize('input,y,epsilon,Exp_LR,epsilon_op,rho', read_parameters())

def test_CompleteANN_RMSProp_Optimizer(input,y,epsilon,Exp_LR,epsilon_op,rho):
   '''
   ========================================================================
   Description:
   ...
   optimizer = RMSProp_Optimizer(learning_R = 1e-3, learning_R_decay = 1e-4, epsilon = 1e-7, rho = 0.9) 
   ...
   # Checking RMSP Optimizer for weight updates in Dense layer 1
   optimizer_RMSP(optimizer,dense_layer_1,Exp_LR,GC_weightsDL1,GC_biasesDL1,epsilon_op,rho)
   
   # Checking RMSP Optimizer for weight updates in Dense layer 2
   optimizer_RMSP(optimizer,dense_layer_2,Exp_LR,GC_weightsDL2,GC_biasesDL2,epsilon_op,rho)

   # Checking RMSP Optimizer for weight updates in Dense layer 3
   optimizer_RMSP(optimizer,dense_layer_3,Exp_LR,GC_weightsDL3,GC_biasesDL3,epsilon_op,rho)
