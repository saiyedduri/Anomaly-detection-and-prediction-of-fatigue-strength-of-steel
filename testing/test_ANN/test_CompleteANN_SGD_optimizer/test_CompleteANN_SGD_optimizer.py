...
def optimizer_learningRate(optimizer,ExpSGD_Current_LR):
   ...
   ========================================================================
   '''
   SGD_Current_LR = optimizer.learning_R_update()
   
   # Assertion
   #------------------------------------------------------------------------
   assert abs(SGD_Current_LR - ExpSGD_Current_LR) < 0.0001, 'test failed' 

def optimizer_SGD(optimizer,Dense_layer,ExpSGD_Current_LR,GradCheck_weightsBP,GradCheck_biasesBP):
   '''
   ========================================================================
   Description:
   ...
   SGD_DL_Weights,SGD_DL_biases = Dense_layer.weights, Dense_layer.biases
   
   # Optimization computed using numerical gradient results
   #------------------------------------------------------------------------
   ExpSGD_DL_Weights = np.ones((Dense_layer.weights.shape)) -ExpSGD_Current_LR * GradCheck_weightsBP
   ExpSGD_DL_biases = 1 -ExpSGD_Current_LR * GradCheck_biasesBP
   
   # Assertion
   #------------------------------------------------------------------------
   np.testing.assert_array_almost_equal(SGD_DL_Weights ,ExpSGD_DL_Weights), 'test failed'
   np.testing.assert_array_almost_equal(SGD_DL_biases ,ExpSGD_DL_biases), 'test failed'
...
def read_parameters():
   ...
   if cwd == 'test_CompleteANN_SGD_optimizer':                    # If tested from the current directory
      filename = 'parameters.txt'
   elif cwd == 'test_ANN':                                        # If tested from test_ANN
      open_path = os.path.abspath('test_CompleteANN_SGD_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
   elif cwd == 'test_ML':                                         # If tested from test_ML
      open_path = os.path.abspath('test_ANN/test_CompleteANN_SGD_optimizer')           
      filename = os.path.join(open_path,'parameters.txt')
...
def test_CompleteANN_SGD_Optimizer(input,y,epsilon,Exp_LR):
   '''
   ========================================================================
   Description:
   ...
   # Checking SGD Optimizer for weight updates in Dense layer 1
   optimizer_SGD(optimizer,dense_layer_1,Exp_LR,GC_weightsDL1,GC_biasesDL1)
   
   # Checking SGD Optimizer for weight updates in Dense layer 2
   optimizer_SGD(optimizer,dense_layer_2,Exp_LR,GC_weightsDL2,GC_biasesDL2)

   # Checking SGD Optimizer for weight updates in Dense layer 3
   optimizer_SGD(optimizer,dense_layer_3,Exp_LR,GC_weightsDL3,GC_biasesDL3)
